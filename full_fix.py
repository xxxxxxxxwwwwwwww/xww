...
def main():
    ...
    # 2. 替换选择题数据
    # 找到选择题开始位置
    start_idx = content.find('// 选择题数据')
    end_idx = content.find('// 问题数据', start_idx)
    ...
    content = content.replace('移动平均预测', 'Python：多维度统计')
    content = content.replace('指数平滑预测', 'SQL窗口函数：累计销售额')
    content = content.replace('EOQ经济订货量', 'SQL子查询：高价值客户')
    content = content.replace('安全库存计算', 'Python：RFM用户分层')
    content = content.replace('供应商选择', 'Python：ABC商品分类')
    content = content.replace('供应商评估', 'Python：时间序列移动平均')
    ...
    print("✅ 完整修复完成！")
    print("主要修复内容：")
    print("1. 替换了页面标题和基本文本")
    print("2. 更新了选择题数据为商务智能相关")
    print("3. 更新了左侧列表题目名称")
    print("4. 更新了编程题和章节标题")
