...
def update_course(html_file, config):
    ...
    # 3. 更新左侧章节导航
    chapter_start = html_content.find('            &lt;h2 class="section-title"&gt;')
    chapter_end = html_content.find('            &lt;/div&gt;\n        &lt;/div&gt;\n', chapter_start) + len('            &lt;/div&gt;\n        &lt;/div&gt;\n')
    
    old_chapter_html = html_content[chapter_start:chapter_end]
    new_chapter_html = generate_chapter_html(config['chapters'], config['title'])
    
    html_content = html_content[:chapter_start] + new_chapter_html + html_content[chapter_end:]
