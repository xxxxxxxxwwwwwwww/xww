...
def update_course(html_file, config):
    ...
    right_nav_start = html_content.find('          &lt;!-- 右侧：问题、代码编辑器、运行结果、答案解析 --&gt;')
    ...
    # 找到章节标题的位置
    section_title_start = html_content.find('            &lt;h2 class="section-title"&gt;', left_nav_start)
    if section_title_start == -1:
        section_title_start = left_nav_start
    
    old_nav_html = html_content[section_title_start:right_nav_start]
