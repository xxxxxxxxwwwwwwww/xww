...
def update_course(html_file, config):
    ...
    # 3. 更新左侧章节导航
    # 找到题库标签页中的左侧导航
    question_bank_start = html_content.find('&lt;!-- 题库标签页 --&gt;')
    wrong_questions_start = html_content.find('&lt;!-- 错题标签页 --&gt;', question_bank_start)
    
    if question_bank_start == -1 or wrong_questions_start == -1:
        print(f"❌ 无法找到题库标签页边界")
        return False
    
    # 在题库标签页范围内查找
    left_nav_start = html_content.find('          &lt;!-- 左侧：题目列表/章节目录 --&gt;', question_bank_start, wrong_questions_start)
    right_nav_start = html_content.find('          &lt;!-- 右侧：问题、代码编辑器、运行结果、答案解析 --&gt;', left_nav_start, wrong_questions_start)
    
    if left_nav_start == -1 or right_nav_start == -1:
        print(f"❌ 无法找到章节导航部分")
        return False
    
    # 找到章节标题的位置
    section_title_start = html_content.find('            &lt;h2 class="section-title"&gt;', left_nav_start, right_nav_start)
    if section_title_start == -1:
        section_title_start = left_nav_start
