...
def update_course(html_file, config):
    ...
    # 3. 更新左侧章节导航
    # 查找第一个"题库标签页"后的内容，到第一个"错题标签页"为止
    left_nav_start = html_content.find('          &lt;!-- 左侧：题目列表/章节目录 --&gt;')
    wrong_questions_start = html_content.find('      &lt;!-- 错题标签页 --&gt;')
    
    if left_nav_start == -1 or wrong_questions_start == -1:
        print(f"❌ 无法找到章节导航部分")
        return False
